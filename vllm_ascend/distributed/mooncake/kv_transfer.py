...
from vllm.utils import logger, get_ip, logger, make_zmq_path, make_zmq_socket
...
class KVTransferThread(threading.Thread):
    ...
    def prepare_value(self, start: int, end: int, slot_mapping: torch.Tensor):
        addr_list=[]
        size_list=[]
    ...
    def _handle_request(self, req_meta: dict[str, Any]):
        ...
        for start, end, key in self.token_database.process_tokens(tokens, mask):
            addr, size=self.prepare_value(start, end, slot_mapping)      
            self.m_store.put(key, addr, size)
        self.task_tracker.update_done_task_count(req_id, self.tp_rank)
        self.request_queue.task_done()
...
class KVCacheStoreLayerSendingThread(KVTransferThread):
    ...
    def _handle_request(self, req_meta: dict[str, Any]):  #chunk
        ...
        for index, key in enumerate(req_meta.keys):
            addr, size=self.prepare_value_layer(req_meta.starts[index], req_meta.ends[index], req_meta.slot_mapping, req_meta.layer_id)
            self.m_store.put(key, addr, size)
        if req_meta.layer_id==self.final_layer_id:
            self.task_tracker.update_done_task_count(req_meta.req_id, self.tp_rank)
        self.request_queue.task_done()
