...
class PagedMemNPUConnector(NPUConnectorInterface):
    ...
    def npu_h2d(self, memory_tensor: torch.Tensor, start: int, end: int, **kwargs):
        ...
        kvcaches: List[List[torch.Tensor]] = kwargs["kvcaches"]
...
class PagedMemNPUConnectorMLA(NPUConnectorInterface):
    ...
    def multi_layer_kv_transfer_py(
        self,
        key_value: torch.Tensor,          # [2, num_layers, num_tokens, scalars_per_token]
        key_value_ptrs: list[torch.Tensor],  # [num_layers][2], tensor [page_buffer_size, 2, scalars_per_token/2]
        slot_mapping: torch.Tensor,       # [num_tokens]
        device: torch.device,
        page_buffer_size: int,
        direction: bool,                  # True: Paged->LMCache, False: LMCache->Paged
        ...
        # Flatten paged buffer once
        flat_kv_ptrs = [ptr.view(-1) for ptr in key_value_ptrs] 
        kv_total = num_layers * num_tokens * scalars_per_token
        for token_id in range(num_tokens):
            slot_idx = slot_mapping[token_id].item()
            if slot_idx < 0:
            ...
            for layer_id in range(num_layers):
                for k_or_v in [0]:  # 0=Key, 1=Value
                    ...
                    # Get slice views
                    lmcache_slice = flat_key_value[lmcache_offset : lmcache_offset + scalars_per_token]
                    paged_slice = flat_kv_ptrs[layer_id][vllm_offset : vllm_offset + scalars_per_token]
                    # Perform copy
                    if direction:  # Paged -> LMCache
                        lmcache_slice.copy_(paged_slice)
                    else:          # LMCache -> Paged
    ...
    def layer_kv_transfer_py(
        self,
        key_value: torch.Tensor,          # [1, num_layers, num_tokens, scalars_per_token]
        kv_caches: torch.Tensor,          # [block_num,block_size,num_heads, head_size]
        slot_mapping: torch.Tensor,       # [num_tokens]
        device: torch.device,
        page_buffer_size: int,
        direction: bool,                  # True: Paged->LMCache, False: LMCache->Paged
        ...
        for token_idx in range(num_tokens):
            ...
            if direction:
                key_value[0, 0, token_idx] = \
                    kv_caches[(int)(slot_idx//block_size), (int)(slot_idx%block_size)].reshape(-1)
            else:
                kv_caches[(int)(slot_idx//block_size), (int)(slot_idx%block_size)] = \
                    key_value[0, 0, token_idx].reshape(num_heads, head_size)
